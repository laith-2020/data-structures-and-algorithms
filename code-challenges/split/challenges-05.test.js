...
const templateWithJQuery = () => {
        // Solution code here...
        starWarsPeople.forEach((item, idx) => {
            let temp = $('#template').clone();
            console.log(item);

            temp.children('h2').text(item.name);
            temp.children('h3').text(item.height);
            temp.children('p').text(item.eye_color);
            $('main').append(temp);
        });
...
const listFoods = (recipe) => {
    let result = [];
    // Solution code here...
    let ingreds = gruffaloCrumble.ingredients;
    // let test = element.slice(element.indexOf( ' ', element.indexOf( ' ' ) +1)+1);

    ingreds.forEach((item, idx) => {
